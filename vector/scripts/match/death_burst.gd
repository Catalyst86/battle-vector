...
## outward, fading over 0.45s. Pooled via SpawnPool — `reset()` clears state
## between uses; `setup()` populates it for a fresh burst.
...
func _draw() -> void:
	...
	var ease: float = 1.0 - pow(1.0 - progress, 2.0)
